...
function calculateLoan(interest, principal, terms){
    var monthly = ((parseFloat(principal * (interest/100))) + parseFloat(principal)) / terms ;
...
function payLoan(that){
    id = $(that).attr('data-id');
    amount = $(that).attr('data-amount');
    payment_no = $(that).attr('data-payment');
    monthly = $(that).attr('data-monthly');
    penalty = $(that).attr('data-penal');

    $('#pment_id').val(id);
    $('#monthlyp').val(numberWithCommas(monthly));
    $('#princ').val(amount);
    $('#penalval').val(penalty);
    $('#pterms').val(payment_no);
    $('#pprincipal').val(amount);
    $('#total_payment').val(numberWithCommas(monthly));

    $('#penal').val('');
    $('#payment_no').html('');
    select = document.getElementById('payment_no');
    for (var i = 1; i<=payment_no; i++){
