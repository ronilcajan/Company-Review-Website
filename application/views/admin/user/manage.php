                            ...
                            <?php foreach ($users as $user) : ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php else : ?>
                                            <div class="avatar avatar-xs">
                                                <img class="avatar-img rounded-circle" alt="user" src="<?= preg_match('/data:image/i', $user->avatar) ? $user->avatar : site_url() . 'assets/uploads/' . $user->avatar ?>" />
                                            </div>
                                        <?php endif ?>
                                        <?= htmlspecialchars($user->first_name, ENT_QUOTES, 'UTF-8') . ' ' . htmlspecialchars($user->last_name, ENT_QUOTES, 'UTF-8') ?>
                                    </td>
                                    <td><?= htmlspecialchars($user->email, ENT_QUOTES, 'UTF-8'); ?></td>
                                    <td><?= htmlspecialchars(date('M. d, Y', strtotime($user->birthdate)), ENT_QUOTES, 'UTF-8'); ?></td>
                                    <td><?= htmlspecialchars($user->company, ENT_QUOTES, 'UTF-8'); ?></td>
